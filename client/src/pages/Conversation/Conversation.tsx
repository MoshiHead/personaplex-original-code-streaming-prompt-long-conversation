...
export const Conversation:FC<ConversationProps> = ({
  ...
  const onDisconnect = useCallback(() => {
    // Fires on every socket close, including ones we're about to automatically recover
    // from (see `onReconnectExhausted` below) -- just tear down the mic for now, the
    // recording pipeline re-attaches itself once a new socket reaches "connected"
    // (see the socket-dependent effect in `useServerAudio`).
    console.log("on disconnect (recording will restart automatically on reconnect)");
    stopRecording();
  }, []);

  const onReconnectExhausted = useCallback(() => {
    // Only reached after repeated automatic reconnect attempts have failed (see
    // RECONNECT_MAX_ATTEMPTS in useSocket.ts) -- at that point we truly give up and
    // require the user to start a new conversation.
    console.log("reconnect attempts exhausted, ending conversation");
    setIsOver(true);
  }, [setIsOver]);
